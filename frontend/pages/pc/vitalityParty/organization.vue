    ...
    <el-card class="content">
      ...
      <el-container>
        <el-aside>
          ...
          <div v-for="item in meetingType" :key="item.id"
               :class="item.id === page.meetingTypeId ? 'activityType' : 'type'" @click="changeType(item.id)">
          ...
          <el-main class="main">
            <el-card v-for="(item, index) in  list" :key="index" class="content">
              <div @click="handleLook(item)">
                ...
                <el-tag :type="item.isQrcode * 1 === 1 ? 'warning' : 'info'">{{ item.isQrcode * 1 === 1 ? '需二维码签到' :
                  '无需二维码签到' }}
                </el-tag>
