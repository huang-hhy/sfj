...
import {
  getDemocraticCateList,
  getDemocraticCate,
  saveDemocraticCate,
  updateDemocraticCate,
  deleteDemocraticCate
} from '@/api/work'
export default {
  name: 'OrgLifeCate',
  ...
  methods: {
    ...
    del(id) {
      deleteDemocraticCate(id).then(res => {
        if (res.code === 0) {
    ...
    updateStatus(id, status) {
      updateDemocraticCate({ id, status }).then(res => {
        if (res.code === 0) {
    ...
    doSubmit(data) {
      if (data.id) {
        updateDemocraticCate(data).then(res => {
          if (res.code === 0) {
            this.$message.success('保存成功')
            this.getList()
          } else {
        ...
        saveDemocraticCate(data).then(res => {
          if (res.code === 0) {
    ...
    getList() {
      ...
      getDemocraticCateList({ pageNum, pageSize }, { typeName: this.page.typeName }).then(res => {
        if (res.code === 0) {
