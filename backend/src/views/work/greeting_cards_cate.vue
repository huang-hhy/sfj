...
import {
  getGreetingCardsCateList,
  getGreetingCardsCate,
  saveGreetingCardsCate,
  updateGreetingCardsCate,
  deleteGreetingCardsCate
} from '@/api/work'
export default {
  name: 'OrgLifeCate',
  ...
  methods: {
    ...
    edit(id) {
      getGreetingCardsCate(id).then(res => {
        if (res.code === 0) {
          const data = res.data
    ...
    del(id) {
      deleteGreetingCardsCate(id).then(res => {
        if (res.code === 0) {
    ...
    updateStatus(id, status) {
      updateGreetingCardsCate({ id, status }).then(res => {
        if (res.code === 0) {
    ...
    doSubmit(data) {
      if (data.id) {
        updateGreetingCardsCate(data).then(res => {
          if (res.code === 0) {
        ...
        saveGreetingCardsCate(data).then(res => {
          if (res.code === 0) {
    ...
    getList() {
      ...
      getGreetingCardsCateList({ pageNum, pageSize }, { typeName: this.page.typeName }).then(res => {
        if (res.code === 0) {
