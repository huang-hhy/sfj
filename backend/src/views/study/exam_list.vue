<template>
  <el-card class="box-card">
    ...
    <el-table :data="tableData" border>
      ...
      <!--          <el-table :data="JSON.parse(scope.row.moldJson)">-->
      ...
      <el-table-column align="center" label="试卷总分" width="80">
        <template slot-scope="scope">{{ scope.row.totalScore ? scope.row.totalScore : '-' }}</template>
      </el-table-column>
...
import AnswerList from '@/views/study/answer_list'
import {
  getExamList,
  getExam,
  saveExam,
...
import { getRoleList, queryOrgTree,getUserSelectList } from '@/api/permission'
...
export default {
  ...
  methods: {
    check(obj) {
      this.answerItem = {
        id: '',
        refresh: !this.answerItem.refresh,
        param: obj
      }
    },
    getOrgTree() {
